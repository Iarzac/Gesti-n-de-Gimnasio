...
# Función para conectar a la base de datos SQLite (por defecto usa gym_db.db)
def conectar_db(db_name="gym_db.db"):
    try:
        conexion = sqlite3.connect(db_name)
        # Para poder usar diccionarios como filas (opcional)
...
def punto_de_venta():
    ...
    prod_dict = {prod["nombre"]: prod["id"] for prod in productos}
    precio_dict = {prod["nombre"]: prod["precio"] for prod in productos}
    ...
    if st.session_state.cart:
        ...
        if st.button("Confirmar Compra"):
            ...
            cursor.execute("INSERT INTO ingresos (tipo, cantidad, fecha) VALUES (?, ?, ?)",
                           ("ventas del gym", total_acumulado, datetime.now().isoformat()))
            conexion.commit()
            cursor.close()
            conexion.close()
...
def reordenar_productos():
    ...
    # SQLite no admite SET FOREIGN_KEY_CHECKS, así que simplemente actualizamos
    for old_id, new_id in mapping.items():
        cursor.execute("UPDATE productos SET id = ? WHERE id = ?", (new_id, old_id))
    conexion.commit()
    cursor.close()
    conexion.close()
...
def reordenar_clientes():
    ...
    # Actualizamos los IDs de clientes
    for old_id, new_id in mapping.items():
        cursor.execute("UPDATE clientes SET id = ? WHERE id = ?", (new_id, old_id))
    # Actualizamos los IDs en inscripciones
    for old_id, new_id in mapping.items():
        cursor.execute("UPDATE inscripciones SET cliente_id = ? WHERE cliente_id = ?", (new_id, old_id))
    # Actualizamos los IDs en pagos
    for old_id, new_id in mapping.items():
        cursor.execute("UPDATE pagos SET cliente_id = ? WHERE cliente_id = ?", (new_id, old_id))
    conexion.commit()
    cursor.close()
    conexion.close()
...
def gestion_clientes():
    ...
    if subopcion == "Clientes":
        ...
        df_clientes = pd.DataFrame(clientes, columns=["ID", "Nombre", "Teléfono", "Dirección", "Notas"])
        ...
        if accion == "Editar Cliente":
            if st.button("Cargar Datos"):
                ...
                if conexion:
                    cursor = conexion.cursor()
                    cursor.execute("SELECT nombre, telefono, direccion, notas FROM clientes WHERE id = ?", (cliente_id,))
            ...
            if st.button("Actualizar Cliente"):
                ...
                cursor.close()
                cursor = conexion.cursor()
                cursor.execute("DELETE FROM inscripciones WHERE cliente_id = ?", (cliente_id,))
                conexion.commit()
                cursor.close()
                for actividad_nombre in actividades_nuevas:
                    ...
                    if conexion2:
                        ...
                        cur.execute("SELECT id FROM actividades WHERE nombre = ?", (actividad_nombre,))
                        row = cur.fetchone()
                        if row:
                            actividad_id = row[0]
                            cur.execute("INSERT INTO inscripciones (cliente_id, actividad_id) VALUES (?, ?)", (cliente_id, actividad_id))
                            conexion2.commit()
                        cur.close()
                        conexion2.close()
        ...
        if actividades_cliente:
            ...
            if st.button("Actualizar Pago"):
                ...
                if conexion is None:
                    return
                cursor = conexion.cursor()
                cursor.execute("SELECT id FROM pagos WHERE cliente_id = ? AND actividad = ?", (cliente_id_actualizar, actividad_actualizar))
                result = cursor.fetchone()
                if result:
                    pago_id = result[0]
                    cursor.execute("UPDATE pagos SET cantidad = ?, fecha_pago = ? WHERE id = ?",
                                   (monto_pagado, proximo_pago, pago_id))
                else:
                    cursor.execute("INSERT INTO pagos (cliente_id, actividad, cantidad, fecha_pago) VALUES (?, ?, ?, ?)",
                                   (cliente_id_actualizar, actividad_actualizar, monto_pagado, proximo_pago))
                conexion.commit()
                cursor.execute("INSERT INTO ingresos (tipo, cantidad, fecha) VALUES (?, ?, ?)",
                               ("pagos de membrecias", monto_pagado, datetime.now().isoformat()))
                conexion.commit()
                cursor.close()
                conexion.close()
                ...
                if conexion:
                    ...
                    cursor.execute("""
                        ...
                        WHERE p.cliente_id = ? AND p.fecha_pago < ?
    ...
    if actividades:
        ...
        if st.button("Registrar Cliente"):
            ...
            if conexion is None:
                return
            cursor = conexion.cursor()
            cursor.execute("INSERT INTO clientes (nombre, telefono, direccion, notas) VALUES (?, ?, ?, ?)",
            ...
            for actividad_nombre in actividades_seleccionadas:
                cursor = conexion.cursor()
                cursor.execute("SELECT id FROM actividades WHERE nombre = ?", (actividad_nombre,))
                actividad_id = cursor.fetchone()[0]
                cursor.close()
                
                cursor = conexion.cursor()
                cursor.execute("INSERT INTO inscripciones (cliente_id, actividad_id) VALUES (?, ?)",
    ...
    if subopcion == "Ver Actividades":
        ...
        if actividades:
            for actividad in actividades:
                ...
                cursor = conexion.cursor()
                cursor.execute("SELECT c.nombre FROM clientes c JOIN inscripciones i ON c.id = i.cliente_id WHERE i.actividad_id = ?", (actividad[0],))
                clientes = cursor.fetchall()
                cursor.close()
        ...
        if st.button("Agregar Actividad"):
            ...
            if conexion is None:
                return
            cursor = conexion.cursor()
            cursor.execute("INSERT INTO actividades (nombre, descripcion, precio, periodo) VALUES (?, ?, ?, ?)",
        ...
        if st.button("Actualizar Actividad"):
            ...
            if conexion is None:
                return
            cursor = conexion.cursor()
            cursor.execute("UPDATE actividades SET nombre = ?, descripcion = ?, precio = ?, periodo = ? WHERE id = ?",
...
def registro_ingresos():
    ...
    if st.button("Registrar Ingreso"):
        ...
        cursor = conexion.cursor()
        cursor.execute("INSERT INTO ingresos (tipo, cantidad, fecha) VALUES (?, ?, ?)",
                       (tipo_ingreso, cantidad, datetime.now().isoformat()))
        conexion.commit()
        cursor.close()
        conexion.close()
    ...
    if st.button("Ejecutar Acción"):
        ...
        if accion == "Eliminar":
            ...
            if ingreso_sel.empty:
                ...
                opciones_tipo = ["Actividades personalisadas", "ventas del gym", "pagos de membrecias"]
                new_tipo = st.selectbox("Nuevo Tipo", opciones_tipo, index=opciones_tipo.index(current_tipo))
                ...
                if st.button("Confirmar Edición", key="editar_ingreso"):
                    cursor.execute("UPDATE ingresos SET tipo = ?, cantidad = ?, fecha = ? WHERE id = ?",
                                   (new_tipo, new_cantidad, datetime.now().isoformat(), ingreso_id))
                    conexion.commit()
...
def gestion_productos():
    ...
    if st.button("Agregar Producto"):
        ...
        if conexion is None:
            return
        cursor = conexion.cursor()
        cursor.execute("INSERT INTO productos (nombre, tipo, precio) VALUES (?, ?, ?)",
    ...
    eliminar_producto = st.selectbox("Seleccione el producto a eliminar", options=[])
    conexion = conectar_db()
    if conexion:
        cursor = conexion.cursor()
